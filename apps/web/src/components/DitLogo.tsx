/**
 * Dit logo with continuously rotating outer and inner rings.
 * Mirrors apps/ios/src/components/DitLogo.tsx: outer ring rotates 360° in 30s,
 * inner ring counter-rotates 360° in 20s. Center dot is static. Animations
 * pause under `prefers-reduced-motion: reduce` (CSS-only).
 */
export function DitLogo({ className }: { className?: string }) {
  return (
    <svg
      className={`dit-logo${className ? ` ${className}` : ''}`}
      width="806"
      height="806"
      viewBox="0 0 806 806"
      fill="none"
