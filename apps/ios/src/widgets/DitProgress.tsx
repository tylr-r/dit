...
const DitProgressLayout = (
  ...
  const todayLine = `${todayCorrect}/${goal} correct today`

  const heroLabel = props.heroKind === 'wpm' ? 'Best WPM' : 'Letters learned'
  ...
  const bestWpm = props.bestWpm ?? 0
  const hasBestWpm = props.heroKind === 'mastered' && bestWpm > 0
  const wpmLine = `${Math.round(bestWpm)} WPM`
  ...
  if (env.widgetFamily === 'accessoryRectangular') {
    ...
    const heroTotal = props.heroTotal ?? 36
    const masteredProgress =
      props.heroKind === 'mastered' && heroTotal > 0
        ? Math.min(Math.max((props.heroValue ?? 0) / heroTotal, 0), 1)
        : 0
    const todayProgress =
      goal > 0 ? Math.min(Math.max(todayCorrect / goal, 0), 1) : 0
    const titleLine = hasStreak ? streakLine : heroValueText
    const subtitleLine = hasStreak ? todayLine : heroLabel
    const progress = hasStreak ? todayProgress : masteredProgress
    return (
      <VStack alignment="leading" spacing={2}>
        ...
        <Text modifiers={[font({ size: 11 })]}>{subtitleLine}</Text>
        ...
        >
          ...
          <HStack alignment="bottom" spacing={16}>
            <VStack alignment="leading" spacing={4}>
              ...
              <Text
                modifiers={[font({ size: 14 }), foregroundStyle(TEXT_MUTED)]}
              >
                {heroLabel}
              </Text>
            </VStack>
