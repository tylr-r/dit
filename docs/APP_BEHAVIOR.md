...
Practices a realistic CW QSO against an LLM playing the other station. iOS adoption deferred.

- Conversation has its own `/app/qso` route. Selecting the mode updates the URL, and refreshing that route keeps Conversation selected. Loading the route is silent: no shared Practice audio may start behind the Conversation surface. Audio begins only after the operator chooses a direction, replays a turn, or keys.
- Idle asks whether the operator wants to **Send first** or **Receive first**. Either direct click unlocks Web Audio. Receive first requests and plays the other station's opener; Send first opens the Morse key immediately so the user can call CQ. A later audio retry replaces any stale pending suspended-context resume. Each contact gives the other station a locally generated US-style practice callsign that remains fixed for that QSO and cannot immediately repeat in the next one. The other station learns the user's name and callsign only from what the user keys during the contact.
- The other station's turns are generated by an LLM (Gemini via Firebase AI Logic), encoded to Morse with the same `textToMorseCode` custom-Listen uses, and played at the user's global playback WPM and tone frequency. Conversation uses that WPM for both character timing and standard Morse spacing rather than inheriting Listen's adaptive Farnsworth effective speed.
- During playback, **Show letters as received** optionally reveals each character when its Morse tones finish. The reveal uses the same AudioContext clock as playback, so pause and replay stay synchronized. The full received plaintext stays concealed in the transcript while audio plays. With live letters off, it remains concealed after playback until the operator reveals it, checks copy, or begins keying a response. With live letters on, it becomes visible when playback finishes.
- The user can type copy during playback. When the transmission ends, the Morse key becomes active immediately and appears before copy controls. Empty copy collapses to a compact optional row below the key; copy already typed during playback stays expanded. Copy never gates the reply. The operator may expand it, check it, hide it, replay the transmission, or ignore it and key immediately. Checked copy uses the same matched/missed/extra comparison as custom Listen and displays the actual expected characters for omissions, including trailing misses.
- A compact **Prosigns** edge tab opens a non-modal side reference on desktop and a bottom sheet on narrow screens. It lists common QSO prosigns and plain-language meanings without showing visual dot-and-dash patterns. The operator can leave it open while keying.
- The user replies by keying the Morse key; dits/dahs decode into a running draft line the same way Freestyle's word mode accumulates letters. Space works as a straight key. Conversation, Practice, and Freestyle all use the same core paddle engine for timed symbols, held-key repeat, paddle switching, and cancellation. The web browser maps left/right Control or bracket keys once, then routes the input to the active mode. **Backspace** and **Clear** edit the draft; **Send** submits it as the next turn.
- A line is only added to the visible transcript once it has actually played (their turn) or been sent (the user's turn) — nothing is revealed early, so a reply is always grounded in what was actually keyed on both sides.
- **Pause** / **Resume** / **Replay** control the currently-playing turn, same semantics as Listen's custom text. **End QSO** stops the session and shows the full transcript; **New QSO** starts over with a fresh callsign and persona.
- If the LLM call fails, the surface shows an error with **Retry**, which resends the same reply.
- The LLM is instructed to use real CW operating conventions (CQ, RST, Q-codes, 73/SK) at a casual, learner-appropriate pace. Perfect `599` reports are sent as the CW cut-number form `5NN`; returned model text is normalized so this remains deterministic. The browser's local hour supplies the appropriate `GM`, `GA`, `GE`, or `GN` greeting context when a QSO starts. A CQ answer is limited to the callsign exchange and handoff. Later turns introduce one exchange unit at a time rather than front-loading RST, name, QTH, and station details. See `docs/DECISIONS.md` for why vocabulary isn't restricted to the user's unlocked letter set here.

**No tracking.** Conversation content and scores are never written to Firebase, and the mode does not affect `dailyActivity`, `letterAccuracy`, `listenTtr`, or streak — same precedent as custom Listen text.
