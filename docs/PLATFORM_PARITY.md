...
| Activation milestones (`first_mode_session` / `first_correct_letter`) | ✅ | ✅ | Web gates once per install via localStorage; iOS gates once per install via AsyncStorage plus an in-memory race guard |
