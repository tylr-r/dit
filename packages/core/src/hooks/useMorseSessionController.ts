...
/**
 * Content shape for guided-course phase transition modals. Platforms may render
 * this however they like (native modal on iOS, web dialog, etc.).
 */
...
/** Options for playing a morse tone sequence. Platforms inject their own audio engine. */
export type PlayMorseToneOptions = {
  code: string
  characterWpm?: number
  effectiveWpm?: number
  minUnitMs?: number
...
/** Options for a single hold/tone. */
export type PlayTonePatternOptions = {
  frequency?: number
  volume?: number
}

/**
 * Callbacks a platform must inject for audio, analytics, and tone preparation.
 * These are native on iOS (ExpoModules) and Web Audio on web, so they live outside core.
 */
export type UseMorseSessionControllerCallbacks = {
  logAnalyticsEvent: (event: string, params?: Record<string, unknown>) => void
  ensureNotificationPermission: () => Promise<boolean>
  prepareToneEngine: () => void | Promise<unknown>
  startTone: (options: PlayTonePatternOptions) => void | Promise<unknown>
...
export type UseMorseSessionControllerOptions = {
  ...
  callbacks: UseMorseSessionControllerCallbacks
}

/**
 * Owns the practice / freestyle / listen session state machine and its sync side effects.
 *
 * This hook is platform-agnostic: audio, analytics, notification permission, and app
 * lifecycle events are injected via adapters (see `usePlatform`) and the `callbacks`
 * option, so the same controller drives the iOS and web apps.
 */
export const useMorseSessionController = ({
  ...
  const playListenSequence = useCallback(
    ...
    [playMorseTone, stopListenPlayback, toneFrequency],
  )
  ...
  const recordRetentionAttempt = useCallback(
    (targetLetter: Letter, attemptMode: ActivityMode, isCorrect: boolean) => {
      if (countsAnswer(attemptMode, isCorrect)) {
        const base: Progress = {
          dailyActivity: dailyActivityRef.current,
          streak: streakRef.current,
          letterAccuracy: letterAccuracyRef.current,
        }
        const next = recordCorrectAnswer(base, { letter: targetLetter, mode: attemptMode })
  ...
  const moveIntoGuidedLesson = useCallback(
    (nextPhase: GuidedPhase, nextPackIndex: number, nextProgress: GuidedLessonProgress) => {
      ...
      setShowAbout(false)
      setShowReference(false)
      const lessonMode: Mode = nextPhase === 'listen' ? 'listen' : 'practice'
      setMode(lessonMode)
  ...
  useEffect(() => {
    const unsubscribe = platform.appLifecycle.subscribe((nextState) => {
      if (nextState === 'background') {
        flushPendingSave()
      }
    })
    return unsubscribe
  }, [flushPendingSave, platform.appLifecycle])
